...
public class QrCodeUtil {
    ...
    public static void drawLogoQRCode(File logoFile, File codeFile, String qrUrl, String note) {
        try {
            ...
            if (StringUtils.isNotEmpty(note)) {
                ...
                outg.setFont(new Font("黑体", Font.PLAIN, 30)); // 字体、字型、字号
                ...
                if (strWidth > 399) {
                    ...
                    outg.drawString(note, 200 - strWidth / 2, height + (outImage.getHeight() - height) / 32); // 画文字
                }
                outg.dispose();
                outImage.flush();
                image = outImage;
    ...
    public static void main(String[] args) throws WriterException {
        File logoFile = new File("E:\\sucai\\logo.png"); //logo
        File QrCodeFile = new File("E:\\sucai\\test.png"); //二维码生成地址以及名称格式
//        String url = "https://www.baidu.com/"; // 扫描二维码进入网址
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("appkey", "df33cb8a73a3ae45749bd1ac6ddeb5e4");
        jsonObject.put("restaurantId", 3451);
        String url = jsonObject.toString(); // 扫描二维码进入网址
        String note = "多美丽洞头店";    //标题
        QrCodeUtil.drawLogoQRCode(logoFile, QrCodeFile, url, note);
    }
}